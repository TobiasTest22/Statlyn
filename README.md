...
This script builds C#, runs the native read-only scan, validates memory-map metadata, starts the local API temporarily, checks `/health`, `/connector/status`, `/diagnostics/memory-maps` and `/diagnostics/fm26/snapshot`, prints snapshot status, connector/process/read-only/map status, blocking gate and next action, then stops the API. It verifies that safe snapshots are diagnostic metadata only and that live player reading remains unavailable.
