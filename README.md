...
- A flat black financial-analysis-style React/Tauri analyst workspace for a serious recruitment department style product.
...
- React/Tauri cockpit refinement: flat financial-analysis-style recruitment terminal with stable left navigation, compact search/filter foundation, safe model/status visuals and a large central scout table with no persistent insight panel, no gradients, no fake visuals and no copied external data.
...
- Player Intelligence Analytics Foundation: C# contracts, role-parameter definitions, backend unavailable-state services, fair value engine, safe API DTOs and React/Tauri display modules exist for player profile, radar profile, heatmap, per-90 metrics, Statlyn Fair Value Estimate, expected fit, archetype, similar players, league average comparison and role assessment. Most modules correctly show unavailable states until real safe local inputs exist.
- FM26 connector diagnostics and safe snapshots: `Statlyn.Api` exposes `/connector/status`, `/connector/fm26`, `/diagnostics/fm26`, `/diagnostics/fm26/summary`, `/diagnostics/memory-maps`, `/connector/memory-maps`, `/diagnostics/fm26/snapshot`, `/connector/fm26/snapshot`, `/diagnostics/fm26/snapshot/readiness`, `POST /diagnostics/fm26/snapshots`, `/diagnostics/fm26/snapshots/latest` and `/diagnostics/fm26/snapshots` through safe C# boundaries. These endpoints report connector availability, Windows/platform state, FM process detection, safe process labels, read-only access status, version/build metadata where available, memory-map registry status, safe snapshot gates, persisted audit history and next action. FM process detection, map selection and safe snapshots are diagnostics metadata only. No player data is read or stored; the endpoints do not expose memory addresses, raw offsets, raw snapshots, hidden values, CA or PA.
...
FM26 remains unsupported for player reading. Milestone 3.3 adds a metadata-only memory-map registry loader and validator: templates are not usable, unvalidated maps are not usable, and a validated map only means map metadata is available. Milestone 3.4 adds safe FM26 diagnostic snapshots that explain connector, process, read-only and map readiness gates. Milestone 3.5 persists those safe snapshots as an audit trail. These snapshots do not read players or memory fields; persisted snapshots are safe diagnostic metadata only. First real memory field reads remain a future milestone.
...
The SQLite persistence layer is local-only foundation work. It stores masked players, visible permitted fields, player stats, physical metrics, source metadata, role scores, shortlist workflow rows, scout assignments, qualitative scout reports, Role Lab templates, benchmark definitions, aggregate benchmark snapshots, blocked-field audit metadata, import audit counts and safe FM26 snapshot audit metadata. Imports run inside a transaction and re-imports replace the current stored player snapshot so safe fields, stats and metrics do not duplicate. It does not store raw provider snapshots, hidden FM26 values, raw blocked values, selected player raw benchmark values, memory addresses, raw offsets or native handles.
...
Run the safe FM26 snapshot diagnostics path:

```powershell
.\tools\run-safe-snapshot-diagnostics.ps1
```

This script builds C#, runs the native read-only scan, validates memory-map metadata, starts the local API temporarily, checks `/health`, `/connector/status`, `/diagnostics/memory-maps` and `/diagnostics/fm26/snapshot`, creates one persisted safe snapshot by default, checks latest/history audit endpoints, prints snapshot status, connector/process/read-only/map status, blocking gate, history count and next action, then stops the API. Use `-NoPersist` for preview-only checks. It verifies that safe snapshots are diagnostic metadata only and that live player reading remains unavailable.
