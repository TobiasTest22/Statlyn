...
FM process detection is diagnostics only. Read-only access is diagnostics only. Memory-map registry loading is metadata only. Safe FM26 snapshots are diagnostic metadata only. Persisted snapshots are safe audit metadata only. No player data is read or stored in 3.5. FM26 remains unsupported for player reading.
...
Milestone 3.3 adds the first memory-map registry loader and validator. Templates are not usable, unvalidated maps are not usable, and selected maps are not used for player reading yet. Milestone 3.4 adds safe diagnostic snapshots that report why live reading is blocked. Milestone 3.5 persists those reports as safe diagnostic audit metadata. React/Tauri never calls native connector directly.
